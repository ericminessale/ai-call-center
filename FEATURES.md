...
The same codebase supports two deployment shapes: the canonical clone-and-own (default) and a public hosted demo for non-technical showcasing. Hosted demo gates on a single env flag (`DEMO_MODE=true`); production-shape deployments are completely unaffected.

### Visitor experience

- **No signup, no login.** Landing page replaces the production sign-in form with a "Start demo" button. Click → assigned a real demo agent identity → dropped into the agent dashboard already-online via WebRTC.
- **Persistent demo banner** across the top of the dashboard with the visitor's leased persona name and the demo phone numbers, dial-able with one click on mobile.
- **Onboarding nudges** for first-time visitors: a floating tip below the status pill walks them from Offline → Available → "pick a queue" without leaving them stuck. An inline tip inside the queue dropdown explains what the checkboxes do. Each tip dismisses independently and remembers the dismissal in localStorage.

### Subscriber pool & lease management

- **Pre-provisioned pool of 20 demo personas** (configurable via `DEMO_POOL_SIZE`) with stable names — Alex Sales, Jordan Support, Sam Receptionist, etc. Each has a persistent SignalWire Subscriber for WebRTC.
- **Anonymous session leasing** — visitor lands → backend leases an idle persona to their session cookie (HttpOnly, SameSite=Lax) → mints a Call Fabric token → frontend goes online as that agent.
- **Race-safe atomic claim** via Redis SETNX so two concurrent visitors can't share a persona, regardless of what SignalWire would do server-side.
- **Heartbeat keep-alive** every 60s while the tab is open; lease auto-releases after 5 min idle (configurable via `DEMO_LEASE_TTL_SECONDS`).
- **Graceful release on tab close** via `navigator.sendBeacon('/api/demo/end')` — persona returns to the pool immediately rather than waiting for TTL.
- **Pool exhaustion is honest** — 21st concurrent visitor gets `503 Demo currently full` with a retry message, not a crash or stale assignment.

### Lockdowns

Active only when `DEMO_MODE=true`. Production deployments behave normally.

- **Outbound dial blocked** — the contact "Call" button, the top-right call widget, and any other dial control still render and react to clicks, but submission shows a "not available in demo mode" toast and never actually places a call. Defense in depth: backend `/api/calls/initiate`, `/api/ai/outbound-call`, and `/api/conferences/<n>/dial-out` all return `403 demo_blocked`; frontend `CallFabricContext.makeCall` refuses at the source before WebRTC starts.
- **Contact delete blocked** — visitors can see the delete UI; clicking shows the same toast. Prevents griefing the seeded contact list.
- **Recording disabled by default** — SWML for inbound calls omits `record_call` so visitor conversations aren't persisted as audio.
- **Per-caller-ID inbound ratelimit** — one phone number can only place N calls per hour to the demo line (default 10, configurable). Over-cap callers get a polite "demo limit reached" SWML and hang up cleanly.
- **Demo personas immutable from admin endpoints** — even a real admin JWT can't modify or delete a `role='demo_agent'` user. Demo personas are pool fixtures owned by the seed layer, not human team members.
- **Demo personas hidden from User Management** — admins see only real teammates in the user list.

### Content moderation

Two-tier check on every visitor-typed text field that gets persisted, gated on `DEMO_MODE`:

- **OpenAI moderation** (free, multilingual) when `OPENAI_API_KEY` is set — catches harassment, hate speech, threats, sexual content, self-harm, violence at the semantic layer. Response includes which categories flagged.
- **Local blocklist** runs as baseline regardless of API availability — focused list of canonical profanity + slurs with vowel-tolerant matching (`f@ck`, `f*ck`, `fck`), leet substitution (`pu$$y`, `sh1t`), Unicode normalization (NFKC + zero-width strip, fullwidth letters), and per-letter-spacing collapse (`p u s s y` → `pussy`).
- **Applied to**: `POST/PUT /api/contacts` (firstName, lastName, displayName, company, jobTitle, notes, email), `POST /api/calls/<id>/ai-message` (message body before the AI sees it).
- **User-facing**: rejection returns `422 moderation_blocked` with the OpenAI category labels (or a generic "language we don't allow" for local-blocklist hits). Frontend axios interceptor catches both and shows a single user-visible toast.
...
- **Idempotent** — running the reset twice in a row is a no-op the second time, never crashes.
- **Safe in production** — endpoint returns `{"skipped": "DEMO_MODE not set"}` outside demo mode; cron container can stay running on a clone-and-own deployment without effect.

### DemoShop MCP integration

A pre-bound External Tools gateway ships with the demo so visitors immediately see the Sales / Support AI specialists invoking real tools mid-call (order lookup, return creation, customer history). See "External Tools (MCP Gateway integrations)" above and `demo-mcp/README.md` for swap-for-prod recipes.

### Audit trail for hosted-demo work

Every milestone PR for the hosted-demo path passes through a checklist gate before merge — the universal items + the milestone-specific section in `SECURITY_AUDIT_CHECKLIST.md`. Any failing item blocks merge. Per the architecture principle, security is a first-class concern once the deployment shape goes public.
